...
class CoopGameroom(Gameroom):
    ...
    def join(self, user):
        super().join(user)
        resp_object = {'players': self.get_usernames(), 'avatars':self.get_avatars()}
    ...
    def handle_gameroom_message(self, user, message):
        ...
        if self.is_started and user.name != self.drawer:
    ...
    def draw_word(self, start = False):
        if start:
            eventlet.sleep(5)
