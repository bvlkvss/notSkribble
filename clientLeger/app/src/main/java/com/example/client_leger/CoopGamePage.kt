...
class CoopGamePage : AppCompatActivity() {
    ...
    private lateinit var player3Text: TextView
    private lateinit var player4Text: TextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_coop_game_page)
        ...
        player1Text = findViewById(R.id.tvplayer1)
        player2Text = findViewById(R.id.tvplayer2)
        player3Text = findViewById(R.id.tvplayer3)
        player4Text = findViewById(R.id.tvplayer4)
        avatar1 = findViewById(R.id.coopAvatar1)
        avatar2 = findViewById(R.id.coopAvatar2)
        avatar3 = findViewById(R.id.coopAvatar3)
        avatar4 = findViewById(R.id.coopAvatar4)
        ...
        coopPlayers = intent.getStringArrayListExtra("players")!!
        coopAvatars = intent.getStringArrayListExtra("avatars")!!
        ...
        player1Text.text = coopPlayers[0]
        var avatarname1 = "avatar${coopAvatars.get(0)}"
        var avatarid1 = this.resources
            .getIdentifier(avatarname1, "drawable", this.packageName)
        avatar1.setImageResource(avatarid1)

        player2Text.text = coopPlayers[1]
        var avatarname2 = "avatar${coopAvatars.get(1)}"
        var avatarid2 = this.resources
            .getIdentifier(avatarname2, "drawable", this.packageName)
        avatar2.setImageResource(avatarid2)

        if (coopPlayers.size > 2) {
            player3Text.visibility = View.VISIBLE
            avatar3.visibility = View.VISIBLE
            player3Text.text = coopPlayers[2]
            var avatarname3 = "avatar${coopAvatars.get(2)}"
            var avatarid3 = this.resources
                .getIdentifier(avatarname3, "drawable", this.packageName)
            avatar3.setImageResource(avatarid3)
        }
        if (coopPlayers.size > 3) {
            player4Text.visibility = View.VISIBLE
            avatar4.visibility = View.VISIBLE
            player4Text.text = coopPlayers[3]
            var avatarname4 = "avatar${coopAvatars.get(3)}"
            var avatarid4 = this.resources
                .getIdentifier(avatarname4, "drawable", this.packageName)
            avatar4.setImageResource(avatarid4)
        }
    ...
    private fun startGame() {
        try {
            var response = JSONObject(intent.extras?.get("data").toString())
    ...
    private fun drawSVG(path: String) {
        ...
        for (i in 1 until commands.size) {
            ...
            if (letter == "M" || letter == "L") {
                ...
                mPath.cubicTo(
                    (coord1[0].toDouble() * 1.33333).toFloat(),
                    (coord1[1].toDouble() * 1.33333).toFloat(),
                    (coord2[1].toDouble() * 1.33333).toFloat(),
                    (coord2[2].toDouble() * 1.33333).toFloat(),
                    (coord3[1].toDouble() * 1.33333).toFloat(),
                    (coord3[2].toDouble() * 1.33333).toFloat()
                )
            }
        }
    ...
    private fun setTimer(timeCount: Long) {
        ...
        timer = object : CountDownTimer(timeC, 1000) {
            ...
            override fun onFinish() {
                wordsCounter = 0
                if (SocketInstance.roomName == SocketInstance.username) {
